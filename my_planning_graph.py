...
class PlanningGraph():
    ...
    def create_graph(self):
        ...
        while not leveled:
            # build the graph in action level
            self.add_action_level(level)
            self.update_a_mutex(self.a_levels[level])

            level += 1

            # build the graph in state (literal) level
            self.add_literal_level(level)
            self.update_s_mutex(self.s_levels[level])

            # stop expanding the graph if last two S levels contain the same literals
            if self.s_levels[level] == self.s_levels[level - 1]:
                leveled = True

    def add_action_level(self, level):# add action-level
        ...
        #  Input of a_levels : precondnodes
        #  Output of a_levels : effnodes
        # s_levels[0] --> a_levels[0] --> .... -->  s_levels[n-1]--> a_levels[n-1] --> s_levels[n]

        # add a PgNode_a to A level when precondition literals of that A level is subset of the previous S level
        # ex: precondition literals (precondnodes) of a_levels[0] is subset of s_levels[0]
        ...
        for action in self.all_actions:
            ...
            if set(node_a.prenodes).issubset(self.s_levels[level]):
                ...
                for node_s in self.s_levels[level]:
                    # Connect new A nodes as children of all the S nodes
                    node_s.children.add(node_a)
                    # Connect S nodes to the parent sets of the new A nodes
    ...
    def add_literal_level(self, level): # add state-level
        ...
        #  Input of a_levels : precondnodes
        #  Output of a_levels : effnodes
        # s_levels[0] --> a_levels[0] --> s_levels[1] .... -->  s_levels[n-1]--> a_levels[n-1] --> s_levels[n]

        # add a PgNode_s to S level when S level is effect literals (effnodes) of the previous A level
        # ex: s_levels[1] is effect literals (effnodes) of a_levels[0]

        for node_a in self.a_levels[level-1]:
            for node_s in node_a.effnodes:
                self.s_levels[level].add(node_s)

                # connect new S nodes as children of all the A nodes
                node_a.children.add(node_s)
                # connect A nodes to the parent sets of the new S nodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        '''
        # Check non-common elements between 2 lists
        # set(list1) ^ set(list2)
        non_common_eff1 = list(set(node_a1.action.effect_add) ^ set(node_a2.action.effect_rem))
        non_common_eff2 = list(set(node_a1.action.effect_rem) ^ set(node_a2.action.effect_add))

        return len(non_common_eff1) == 0 &   len(non_common_eff2) == 0
        '''

        # Check common elements between 2 lists
        # list(set(list1) & set(list2))
        # list(set(list1).intersection(list2))

        common_eff1 = list(set(node_a1.action.effect_add) & set(node_a2.action.effect_rem))
        common_eff2 = list(set(node_a1.action.effect_rem) & set(node_a2.action.effect_add))

        #common_eff1 = list(set(node_a1.action.effect_add).intersection(node_a2.action.effect_rem))
        #common_eff2 = list(set(node_a1.action.effect_rem).intersection(node_a2.action.effect_add))

        return (len(common_eff1) > 0) or  (len(common_eff2) > 0)

        # 1
        '''
        # check if node_a1's effect_rem literals appear in node_a2's effect_add literals and vice versa
        if len ([1 for expr in node_a1.action.effect_add if expr in set(node_a2.action.effect_rem)] |
                [1 for expr in node_a1.action.effect_rem if expr in set(node_a2.action.effect_add)]):
            return True
        '''
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        common_eff_precond_1 = list(set(node_a1.action.effect_add) & set(node_a2.action.precond_neg))
        common_eff_precond_2 = list(set(node_a2.action.effect_add) & set(node_a1.action.precond_neg))
        common_eff_precond_3 = list(set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos))
        common_eff_precond_4 = list(set(node_a2.action.effect_rem) & set(node_a1.action.precond_pos))


        #common_eff_precond_1 = list(set(node_a1.action.effect_add).intersection(node_a2.action.precond_neg))
        #common_eff_precond_2 = list(set(node_a2.action.effect_add).intersection(node_a1.action.precond_neg))
        #common_eff_precond_3 = list(set(node_a1.action.effect_rem).intersection(node_a2.action.precond_pos))
        #common_eff_precond_4 = list(set(node_a2.action.effect_rem).intersection(node_a1.action.precond_pos))

        return (len(common_eff_precond_1) > 0) or (len(common_eff_precond_2) > 0) or (len(common_eff_precond_3) > 0) or  (len(common_eff_precond_4) > 0)

        #return len(common_eff_precond_1 | common_eff_precond_2 | common_eff_precond_3 | common_eff_precond_4) > 0

        # 1
        '''
        # Check if node_a1's effect_rem literals appear in node_a2's effect_add literals and vice versa
        if len ([1 for expr in node_a1.action.effect_add if expr in set(node_a2.action.precond_neg)] |
                [1 for expr in node_a2.action.effect_add if expr in set(node_a1.action.precond_neg)] |
                [1 for expr in node_a1.action.effect_rem if expr in set(node_a2.action.precond_pos)] |
                [1 for expr in node_a2.action.effect_rem if expr in set(node_a1.action.precond_pos)]):
            return True
        '''
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for node1_parent, node2_parent in product(node_a1.parents, node_a2.parents):
             if node1_parent.is_mutex(node2_parent):
                    return True
